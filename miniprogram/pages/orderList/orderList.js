// pages/appointList/orderList.js
const app = getApp();
...
Page({
    ...
    setOrderList: function(){
        ...
        let limit = 5;
        OrderDB.GetOrder(app.globalData.openid, this.data.orderList.length, limit).then(res=>{
            // 对时间+8h处理，并转换为string
    ...
    cancel: function(e){
        ...
        // 删除信息
        let content = `订单号: ${uuid}`;
        API.ShowModal("确认取消信息", content).then(res=>{
            wx.showLoading({
              title: '删除中',
            })
            // 删除订单信息
            CloudDB.DeleteWxCloudDB('order_list', {uuid: uuid}, '删除预约订单').then(res=>{
                wx.hideLoading();
    ...
    copyToClipboard: function(e){
        var stringForCopy = `${e.currentTarget.dataset.data}`;
        wx.setClipboardData({
            //准备复制的数据
            data: stringForCopy,
